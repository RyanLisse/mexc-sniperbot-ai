...
describe("Config API", () => {
    ...
    it("rejects non-positive maxTradeUsdt", async () => {
        await expect(updateConfig({ maxTradeUsdt: 0 })).rejects.toThrow(/max_trade_usdt must be positive/i);
        await expect(updateConfig({ maxTradeUsdt: -10 })).rejects.toThrow(/max_trade_usdt must be positive/i);
    });
    it("rejects maxPositionUsdt < maxTradeUsdt when both provided", async () => {
        await expect(updateConfig({ maxTradeUsdt: 200, maxPositionUsdt: 100 })).rejects.toThrow(/max_position_usdt must be >= max_trade_usdt/i);
    });
});
