...
export const listingEventIndexes = {
  ...
  listingTimeIdx: index("listing_events_listing_time_idx").on(listingEvent.listingTime),
  
  // Index for fast lookup by detected time (used for recent listings)
  detectedAtIdx: index("listing_events_detected_at_idx").on(listingEvent.detectedAt),
  ...
  expiresAtIdx: index("listing_events_expires_at_idx").on(listingEvent.expiresAt),
};
...
export const tradeAttemptIndexes = {
  ...
  createdAtIdx: index("trade_attempts_created_at_idx").on(tradeAttempt.createdAt),
  ...
  detectedAtIdx: index("trade_attempts_detected_at_idx").on(tradeAttempt.detectedAt),
  
  // Index for completed trades (used in statistics)
  completedAtIdx: index("trade_attempts_completed_at_idx").on(tradeAttempt.completedAt),
};
...
export const botStatusIndexes = {
  ...
  lastHeartbeatIdx: index("bot_status_last_heartbeat_idx").on(botStatus.lastHeartbeat),
  
  // Index for cleanup queries
  createdAtIdx: index("bot_status_created_at_idx").on(botStatus.createdAt),
};
...
export const tradingConfigurationIndexes = {
  ...
  versionIdx: index("trading_configuration_version_idx").on(
    tradingConfiguration.version
  ),
};
