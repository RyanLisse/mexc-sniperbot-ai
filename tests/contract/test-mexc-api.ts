...
import { MEXCClient } from "../../packages/api/src/services/mexc-client";
import { SigningService } from "../../packages/api/src/services/mexc-signing";
...
describe("MEXC API Contract Tests", () => {
  ...
  describe("Request Signing", () => {
    test("should generate valid HMAC SHA256 signature", () => {
      ...
      const result = Effect.runSync(
        signingService.signRequest(params, secretKey)
      );
    ...
    test("should validate signature format", () => {
      ...
      const secretKey = "another_secret";
      ...
      expect(/^[a-f0-9]{64}$/i.test(result.signature)).toBe(true);
    });
  ...
  describe("API Response Contracts", () => {
    ...
    test("should validate order response structure", () => {
      ...
      expect(mockOrderResponse.side).toMatch(/^(BUY|SELL)$/);
      expect(mockOrderResponse.status).toMatch(/^(NEW|FILLED|PARTIALLY_FILLED|CANCELED)$/);
    });
  ...
  describe("Request Parameter Validation", () => {
    test("should validate symbol format", () => {
      ...
      for (const symbol of invalidSymbols) {
        expect(/^[A-Z]{3,}USDT$/.test(symbol)).toBe(false);
      }
    });
